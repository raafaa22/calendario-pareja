...
export default function Login({ onLogin, error, busy }: Props) {
  const { days } = togetherBreakdown()

  return (
    <div className="flex flex-1 flex-col items-center justify-center gap-6 px-8 text-center">
      <div>
        <div className="mx-auto h-14 w-14 rounded-2xl bg-gradient-to-br from-sky-300 to-emerald-300" />
      ...
      <p className="max-w-xs text-sm leading-relaxed text-white/40">
        Entra con tu cuenta de Google. La app lee y escribe directamente en
        vuestros calendarios: no guarda nada en ningún servidor.
      </p>
      ...
      <button
        ...
        onClick={onLogin}
        disabled={busy}
        className="tap flex items-center gap-2.5 rounded-full bg-white px-5 py-3 text-sm font-semibold text-slate-900 disabled:opacity-60"
      >
      ...
      {error && (
        <p className="max-w-xs rounded-xl border border-red-400/30 bg-red-500/10 px-3 py-2 text-xs leading-relaxed text-red-200">
          {error}
        </p>
      )}
    </div>
